...
import { onShowRecapCreationGame, onCreationGame, handleCloseRecapModal } from '../../../methods/game'
...
class MultiPlayer extends Component {
  ...
  render() {
    ...
    const { show, showRecap, title, errorInput } = this.state
    const showModal = <Modal show={show} onHide={() => handleCloseModal(this.setState.bind(this))}>
      <Modal.Header closeButton>
        <Modal.Title>Création partie multijoueur</Modal.Title>
      </Modal.Header>
    ...
    const showModalRecap =
      <Modal show={showRecap} onHide={() => handleCloseRecapModal(this.setState.bind(this))}>
        ...
        <Modal.Body>
          ...
          <p>Titre de la partie:&nbsp;<span className="name-game">{title}</span></p>
        ...
        <Modal.Footer>
          ...
          <Button variant="success" onClick={() => onCreationGame(this.state, this.setState.bind(this))}>
            Créer la partie
      </Button>
    ...
    return (
      ...
      <section className="col-md-4 col-sm-12 col-xs-12 mt-5  ">
        ...
        <Card className="bg-dark text-white">
          <Card.Title> Multi-Joueur</Card.Title>
          <Card.Img src={othelloPicture} alt="Card image othello" />


          <Card.Footer>
            <button className="multi-player-button" onClick={() => exitModalShow(this.setState.bind(this))}>Créer une nouvelle partie Multi-Joueur</button>
